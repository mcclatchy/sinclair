...
jQuery(document).ready(function() { setTimeout(function () {
    jQuery("#gregbox-outer").css({"position":"absolute","z-index":"100001", "background-color":"white"})
}, 5000)});
...
mi.Search.prototype.searchParamConfig=function(search_type,search_text){this.setConf("searchSelectorType","radio");if(search_type){switch(search_type){case"web_search":this.setConf("form_action","http://search.miamiherald.com/search-bin/search.pl.cgi");this.setConf("query_fields",{sf_Keywords:search_text,product:"Yahoo,Overture",collection:"WEB",live_template:"http://www.miamiherald.com/searchresults/v-ysr/index.html",error_template:"http://www.miamiherald.com/searchresults/v-yerr/index.html",preview_template:"http://preview.miamiherald.com/searchresults/v-ysr/index.html",results_per_page:"10",preview:"0",prop_related:"1",prop_dym:"1"});break;default:this.configErrorReporter();}}};var mi=(!mi)?{'media_domain':''}:mi;mi.SideScrollAd=function(container){this.container=$(container).length?$(container):'';this.trigger_percentage=70;this.show=false;if(this.container!=='')
...
$(window).load(function(){$('div[name=adx_al]').bind('click',function(){var $curMarg=$('body').css('margin-bottom').replace("px","");$curMarg=($curMarg==30)?110:30;$('body').css('margin-bottom',$curMarg+'px');});$('.advertisement img').each(function(index){if(this.height==1&&this.width==1){$(this).css("display","none");}});});/*
...
mi.Commenting.prototype.reset_disqus_config=function(disqus_cookie_val,public_api_key){var mi_disqus_config=new disqus_config();var sso_name=mi_disqus_config.sso.name.toString();var sso_button=mi_disqus_config.sso.button.toString();var sso_url=mi_disqus_config.sso.url.toString();var sso_logout=mi_disqus_config.sso.logout.toString();var sso_width=mi_disqus_config.sso.width.toString();var sso_height=mi_disqus_config.sso.height.toString();disqus_config=function(){this.page.remote_auth_s3=disqus_cookie_val;this.page.api_key=public_api_key;this.sso={name:sso_name,button:sso_button,url:sso_url,logout:sso_logout,width:sso_width,height:sso_height};};}
 ...
 var disqus_config = function () {
     this.sso = {
         name:    "Miami Herald",
         button:  "http://media.miamiherald.com/static/images/dsq-login-button-mi.png",
         url:     "http://www.miamiherald.com/static/insite/disqus_login.html",
         logout:  "http://www.miamiherald.com/reg-bin/tint.cgi?mode=logout",
         width:   "600",
         height:  "375"
     };
 };
...
var dsvalue=data.page.deals.deal.productvalue.$t;var dsprice=data.page.deals.deal.saleprice.$t;var dollarsoff=(dsvalue-dsprice);var percentoff=(dollarsoff/dsvalue)*100;var misitelink=data.page.site.sitelink.$t+lidHash+"#widget=ds_rrail";var mideallink=data.page.deals.deal.link.$t+lidHash+"#widget=ds_rrail";jQuery("#ds_value").html("$"+Math.round(dsvalue));jQuery("#ds_discount").html(Math.floor(percentoff)+"%");jQuery("#ds_save").html("$"+Math.round(dollarsoff));jQuery("#dealsaver_td .ds_title_link").attr("href",mideallink);jQuery("#dealsaver_td .ds_title_link").html(data.page.deals.deal.offer.$t);jQuery("#dealsaver_td .ds_pricetag_container").html("$"+data.page.deals.deal.saleprice.$t);jQuery("#dealsaver_td .ds_deal_image img").attr("src",data.page.deals.deal.splashpagethumbnail.$t);jQuery("#dealsaver_td .ds_logo_link").attr("href",misitelink);jQuery("#dealsaver_td .ds_dealtitle").attr("href",misitelink);jQuery("#dealsaver_td .ds_deal_image a").attr("href",mideallink);jQuery("#dealsaver_td .ds_pricetag a").attr("href",mideallink);};mi.DealSaver.prototype.displayWidget=function(display_mode){var self=this;if(display_mode!==0&&display_mode!==2){jQuery("#dealSaverWidget").attr("style","display:block");}else if(display_mode==2){jQuery("#dealSaverWidget .ds_buycontainer").attr("style","display:none");jQuery("#dealSaverWidget .ds_deal_image").attr("style","float:none; text-align:center");jQuery("#dealSaverWidget").attr("style","display:block");}};if ("undefined" != typeof(mi))
{
    ...
    * */
   mi.paywall.enabled = 1;
   mi.paywall.exclude_section = [1714,1715,1716,1730,1731,1732,1733];
   // for page_types other than *Story and *Gallery
   //mi.paywall.page_type = ["*Vendor"];
}

if (0 == document.domain.toLowerCase().indexOf('preview'))
{
   var mistats = {};
   mistats.pagelevel = "*Story";
}
// aggregated into siteresources.js after paywall.js
jQuery(function() {
console.log("control: " + mi.control.paywall);
console.log("paywall: " + mi.paywall);
...
var isPaywallActive = false;
var enabled;

// if data are missing, or
// if turned off in control.inc, no paywall
if ("undefined" == typeof(mi) ||
    "undefined" == typeof(mi.paywall) ||
    "undefined" == typeof(mi.control) ||
    "undefined" == typeof(mi.control.paywall) ||
    "undefined" == typeof(mistats) ||
    "undefined" == typeof(mistats.pagelevel) ||
    0 == mi.control.paywall)
{
   enabled = 0;
}
else if ("undefined" != typeof(mi.paywall.enabled))
{
   enabled = mi.paywall.enabled;
}
else
{
   enabled = mi.control.paywall;
}
...
if (0 != enabled)
{
   var page_type = ['*Story','*Gallery'];
   if ("undefined" != typeof(mi.paywall.page_type))
   {
      page_type = page_type.concat(mi.paywall.page_type);
   }
   var exclude_section = [];
   if ("undefined" != typeof(mi.paywall.exclude_section))
   {
      exclude_section = mi.paywall.exclude_section;
   }
   var domain = document.domain.toLowerCase();

console.log('mistats: ' + mistats.pagelevel);
console.log('page_type: ' + page_type.indexOf(mistats.pagelevel));

// if section is in exclude_section,
// no paywall, regardless of other tests
   if (page_type.indexOf(mistats.pagelevel) >= 0 &&
       ("undefined" == typeof(mi.pageInfo) ||
        "undefined" == typeof(mi.pageInfo.section) ||
        ("undefined" != typeof(mi.pageInfo.section.id) &&
        -1 == exclude_section.indexOf(mi.pageInfo.section.id))) &&
       (1 == enabled ||
       (0 == domain.indexOf('preview') && [1,2].indexOf(enabled) >= 0)))
   {
      isPaywallActive = true;
   }
}

console.log('isPaywallActive: ' + isPaywallActive);
if (isPaywallActive)
{
   var paywallCodeA = document.createElement('script');
   paywallCodeA.id = 'paywallCodeA';
   paywallCodeA.type = 'text/javascript';
   paywallCodeA.src = 'http://s.ppjol.net/pp.js';
   paywallCodeA.text = "{'zone':'" + mi.paywall.zone_id + "','mode':'meter','debug':0}";

   var headID = document.getElementsByTagName("head")[0];
   headID.appendChild(paywallCodeA);
}
});
if (!Array.prototype.indexOf) {
